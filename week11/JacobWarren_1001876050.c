...
{ 
    ...
    for (int i = 0; i < 3; i++)
    {
        printf("Enter two values for row %d: ", i+1);
        for (int j = 0; j < 2; j++)
        {
            scanf("%d", &q8Array[i][j]);
        }
    ...
    printf("\nInitial Array:\n");
    print2DimensionalArray(3, 2, q8Array);
    
    printf("Enter the row to square: ");
    scanf("%d", &row);
    if (row >= 1 && row <= 3)
    {
        q8Array[row-1][0] = makeSquare(q8Array[row-1][0]);
        q8Array[row-1][1] = makeSquare(q8Array[row-1][1]);
    }
    printf("\nUpdated Array:\n");
    print2DimensionalArray(3, 2, q8Array);
